...
# ======================== MENTAL‑HEALTH PRE‑PROCESS ========================
import pandas as pd, io, re
from google.colab import files

# ── upload ────────────────────────────────────────────────────────────────
print("🔼  Upload the Mental‑Health file (.csv / .xlsx)")
...
# ── keep & rename just the 4 shared columns ───────────────────────────────
col_map = {
...
# optional: convert weekly → daily so it matches the other dataset’s “hours per day”
df["working_hours"] = pd.to_numeric(df["working_hours"], errors="coerce") / 7.0

# ── tidy up stress_level if it’s text (Low / Moderate / High …) ────────────
level_map = {"low": 1, "moderate": 2, "medium": 2, "high": 3, "very high": 4}
df["stress_level"] = (
    pd.to_numeric(df["stress_level"], errors="coerce")   # keep numeric as‑is
      .fillna(df["stress_level"].str.lower().map(level_map))
)

# ── numeric conversion & drop NA rows ──────────────────────────────────────
for c in df.columns:
...
print(f"✅  Saved {out}  — rows kept: {len(df)}")
files.download(out)

# ========================= REMOTE‑WORK PRE‑PROCESS =========================
import pandas as pd, io, re
from google.colab import files

# ── upload ────────────────────────────────────────────────────────────────
print("🔼  Upload the Remote‑Work file (.csv / .xlsx)")
uploaded = files.upload()
fname    = next(iter(uploaded))
