...
# 3. Basic EDA snapshot (optional – quick sanity check)
...
# Treat Work_Life_Balance_Satisfaction as categorical if integer 1‑5
...
# 5. Train‑test split + SMOTE (only on train data)
...
# 6.2 TabNet (attention‑based DL for tabular)
print("\n🚀 Training TabNet… (will take ~1‑2 min on CPU)")
# Convert to numpy
...
def evaluate(model, X_tr, y_tr, X_te, y_te, label="Model"):
    ...
    for split, y_true, y_pred in [("TRAIN", y_tr, y_pred_train), ("TEST", y_te, y_pred_test)]:
        ...
        print(f"{split} — Acc: {acc:.4f} | F1: {f1:.4f} | Prec: {prec:.4f} | Rec: {rec:.4f}")
...
# =============================================================
# 🔄 Extra Models + Unified Metric Table  (0‑based labels fix)
# =============================================================
...
# 2) Shift labels to 0‑based (XGBoost/LightGBM requirement)
y_train_res_adj = y_train_res - y_train_res.min()   # e.g. 1‑10 → 0‑9
...
# 5) Soft‑Voting Ensemble (tree models)
